...
public:
	...
	const glm::ivec2 getDims(){ return glm::ivec2((float)m_gridDims.x, (float)m_gridDims.y); }
	...
	void drawDebug(Sakura::DebugRenderer& debugRenderer){
		for (int i = 0; i < m_gridDims.y; ++i){
			for (int j = 0; j < m_gridDims.x; ++j){
				debugRenderer.drawBox(glm::vec4(j*m_tileDims.x, i*m_tileDims.y, m_tileDims), Sakura::ColorRGBA8(0,255,255,255), 0);
			}
		}
	}
private:
